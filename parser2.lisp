...
;;; LICENSE Free of use and adaptation, provided that this disclaimer is included  


;;; The author is not responsable for any damage caused by usage of this code  
...
;;; - Unbalanced parenthesis are compensated or discarded.  


;;; - Excessive right parentheses are discarded.  


;;; - Excessive left parentheses are compensated with right parentheses.  
...
;;; - Non standard chars are replaced by spaces  
...
( defgeneric parse-with-pretty-node ( node state-previous char-stream ) )


( defmethod parse-with-pretty-node
           ( ( atom-node pretty-atom ) state-previous char-stream )
           ( let ( ( state ) )
             ...
             ;; continue according to the new state
              ( cond 
                   ;;end of stream
                    ( ( not state ) ( setf ( getf state :eof ) t )
                    ( progn
                     ( when ( getf state-previous :concatenation )
...
( defmethod parse-with-pretty-node ( ( list-node pretty-list ) state char-stream )
...
( defun parse-to-pretty-lisp ( text-code &optional ( input-type :string ) )
...
The input-type of text-code can be :stream or :string"
  ( let (
    ...
    ( loop while ( and
                 ( setf state
                         ( parse-with-pretty-node ( make-instance 'pretty-list )
                          '( :char #\Space :separator t ) text-stream ) )
